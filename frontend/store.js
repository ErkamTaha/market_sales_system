...
const store = createStore({
    ...
    mutations: {
        ...
        ADD_TO_CART(state, { product, quantity = 1 }) {
            const existingIndex = state.saleItems.findIndex(item => item.id === product.id);
    ...
    actions: {
        ...
        // Satışı tamamla
        async completeSale({ state, commit, dispatch }) {
            if (state.saleItems.length === 0) return;

            try {
                const saleData = {
                    total_amount: state.saleItems.reduce((total, item) => total + (item.quantity * item.unit_price), 0),
    ...
    getters: {
        ...
        totalCartAmount: (state) => {
            return state.saleItems.reduce((total, item) => total + (item.quantity * item.unit_price), 0);
        },
