// components/Products.js - Ürün Yönetimi Sayfası Component
        ...
        <v-container>
            ...
            <v-card>
                ...
                <v-data-table
                    ...
                    :loading="$store.state.loadingProducts"
                    :items-per-page="10"
                    item-value="id"
                >
                    ...
                    <template v-slot:item.is_active="{ item }">
                        <v-switch
                            ...
                            color="success"
                            hide-details
                            @change="toggleProductActiveness(item)"
                        ></v-switch>
                    </template>
                    <template v-slot:item.is_barcode_easy="{ item }">
                        <v-switch
                            v-model="item.is_barcode_easy"
                            color="success"
                            hide-details
                            @change="toggleProductBarcode(item)"
                        ></v-switch>
                    </template>
            ...
            <v-dialog v-model="showAddProduct" max-width="800">
                <v-card>
                    <v-card-title class="bg-primary text-white">
                        <span class="icon-text">{{ editingProduct ? '✏️' : '➕' }}</span>
                    ...
                    <v-card-text class="pa-6">
                        <v-form ref="productForm" v-model="formValid">
                            <v-row>
                                ...
                                <v-col cols="12" md="6">
                                    <v-text-field
                                        v-model="productForm.critical_stock_level"
                                ...
                                <v-col cols="12">
                                    ...
                                    ></v-switch>
                                    <v-switch
                                        v-model="productForm.is_barcode_easy"
                                        label="Barkod Kolaylığı"
                                        color="success"
                                        inset
                                        hide-details
                                    ></v-switch>
            ...
            <!-- Ürün Detay Modal -->
            <v-dialog v-model="showProductDetail" max-width="600">
                <v-card v-if="selectedProduct">
                    ...
                    <v-card-text class="pa-6">
                        <div class="text-center mb-4">
                            ...
                            <h2 class="mt-3">{{ selectedProduct.name }}</h2>
                            <v-chip :color="selectedProduct.is_active ? 'success' : 'error'">
                        ...
                        <v-list>
                            <v-list-item>
                                ...
                                <v-list-item-subtitle>{{ selectedProduct.id }}</v-list-item-subtitle>
                            </v-list-item>
                            ...
                            <v-list-item>
                                ...
                                <v-list-item-subtitle>{{ selectedProduct.category || 'Kategorisiz' }}</v-list-item-subtitle>
                            </v-list-item>
                            <v-list-item>
                                <v-list-item-title>Fiyat</v-list-item-title>
                                <v-list-item-subtitle class="font-weight-bold">{{ selectedProduct.price.toFixed(2) }} ₺</v-list-item-subtitle>
                            </v-list-item>
                            <v-list-item>
                            ...
                            <v-list-item>
                                ...
                                <v-list-item-subtitle>{{ selectedProduct.critical_stock_level }} {{ selectedProduct.unit }}</v-list-item-subtitle>
                            </v-list-item>
                            <v-list-item v-if="selectedProduct.expiry_date">
                                <v-list-item-title>Son Kullanma</v-list-item-title>
                                <v-list-item-subtitle>
                                    <v-chip :color="getExpiryColor(selectedProduct.expiry_date)" size="small">
                    ...
                    <v-card-actions class="pa-4">
                        <v-spacer></v-spacer>
                        <v-btn @click="showProductDetail = false" variant="outlined">Kapat</v-btn>
                ...
                <v-card>
                    ...
                    <v-card-text class="pa-6" v-if="selectedProductForStock">
                        <div class="text-center mb-4">
                            ...
                            <p class="text-grey">Mevcut: {{ selectedProductForStock.current_stock }} {{ selectedProductForStock.unit }}</p>
                        </div>
                        ...
                        <v-text-field
                            v-model="stockUpdateAmount"
                            :label="getStockUpdateLabel()"
                            type="number"
                            step="0.1"
                            min="0"
                            :suffix="selectedProductForStock.unit"
                        ></v-text-field>
                        <v-alert 
                            v-if="stockUpdateAmount && stockUpdateType !== 'set'"
                            :type="stockUpdateType === 'add' ? 'success' : 'warning'"
                            variant="tonal"
                        >
